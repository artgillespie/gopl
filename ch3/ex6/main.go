...
func main() {
	...
	for py := 0; py < height; py++ {
		for px := 0; px < width; px++ {
			...
			img.Set(px, py, sample(float64(px), float64(py), width, height, xmin, ymin, xmax, ymax))
		}
	}
	png.Encode(os.Stdout, img) // NOTE: ignoring errors
}

func sample(px, py, width, height, xmin, ymin, xmax, ymax float64) color.Color {
	var r, g, b uint32
	var count uint32
	for yy := py - 0.5; yy < py+0.5; yy += 0.5 {
		y := yy/width*(xmax-xmin) + xmin
		for xx := px - 0.5; xx < px+0.5; xx += 0.5 {
			x := xx/height*(ymax-ymin) + ymin
			z := complex(x, y)
			tr, tg, tb, _ := mandelbrot(z).RGBA()
			r += tr
			g += tg
			b += tb
			count++
